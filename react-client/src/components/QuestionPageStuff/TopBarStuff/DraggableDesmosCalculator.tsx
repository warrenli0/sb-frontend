...
const DraggableDesmosCalculator: React.FC<DraggableCalcProps> = ({ showCalc, setshowCalc }) => {
  const [position, setPosition] = useState({ x: 50, y: window.innerHeight - 425 });
  ...
  const handleMouseMove = (e: MouseEvent) => {
    if (draggingRef.current) {
      ...
      const boundedX = Math.max(0, Math.min(newX, window.innerWidth - 600)); // Adjusted for calculator width
      const boundedY = Math.max(44, Math.min(newY, window.innerHeight - 200)); // Adjusted for calculator height
