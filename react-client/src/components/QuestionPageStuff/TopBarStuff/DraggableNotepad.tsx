...
const DraggableNotepad: React.FC<DraggableNotepadCalcProps> = ({ showNote, setshowNote, isCorrect }) => {
  ...
  const [position, setPosition] = useState<{ x: number; y: number }>({ x: 50, y: window.innerHeight - 525 });
  ...
  const handleMouseMove = (e: MouseEvent) => {
    if (!isDragging) return;
        ...
        const boundedX = Math.max(0, Math.min(newX, window.innerWidth - 600));
        
        // Prevent moving off the top or bottom edge
        const boundedY = Math.max(44, Math.min(newY, window.innerHeight - 200));
